...
CKimg_dir = "./dataset/VOC/JPEGImages"
...
def save_annotations(filename, objs, filepath):
    ...
    anno_tree = E.annotation(
        E.folder("1"),
        E.filename(filename),
        E.source(E.database("CKdemo"), E.annotation("VOC"), E.image("CK")),
        E.size(E.width(img.shape[1]), E.height(img.shape[0]), E.depth(img.shape[2])),
        E.segmented(0),
    )
    for obj in objs:
        ...
        anno_tree2 = E2.object(
            ...
            E.truncated("0"),
            E.difficult(0),
            E.bndbox(E.xmin(obj[2]), E.ymin(obj[3]), E.xmax(obj[4]), E.ymax(obj[5])),
        )
    ...
    for ann in anns:
        ...
        if "bbox" in ann:
            ...
            if verbose:
                ...
                cv2.putText(image, name, (xmin, ymin), 3, 1, (0, 0, 255))
...
def get_CK5(origin_anno_dir, origin_image_dir, verbose=False):
    annFile = "train.json"
    annpath = os.path.join(origin_anno_dir, annFile)  # 读取标签目录下的train.json
    coco = COCO(annpath)  #
...
def main():
    ...
    mkr(image_dir)
    mkr(anno_dir)
    origin_image_dir = "./dataset/train/images/"  # step 2原始的coco的图像存放位置
    origin_anno_dir = "./dataset/train/annotations"  # step 3 原始的coco的标注存放位置
